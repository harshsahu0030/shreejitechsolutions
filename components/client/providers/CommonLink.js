...
const CommonLink = ({ label = "", url = "/", target = "" }) => {
  return (
    <Link
      href={url}
      target={target}
      className="group/common-btn p-1 font-semibold uppercase rounded-full flex items-center justify-center transitions cursor-pointer w-fit hover:bg-(--bg-black) border-2 hover:text-(--text-white) hover:border-(--bg-black) h-fit w-fit"
    >
